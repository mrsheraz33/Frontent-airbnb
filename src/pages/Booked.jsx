...
function Booked() {
    ...
    let {getListing ,cardDetails} = useContext(listingData)
    let {getCurrentUser } = useContext(userDataContext)
 
const handelRatings = async (id)=>{
    try {
...
<div className='w-[95%] max-w-[500px] h-[400px] bg-white flex items-center justify-center border-[1px] border-[#b5b5b5]
...
<div className='w-[95%] max-w-[600px] h-[200px] bg-white flex items-center justify-center border-[1px] 
border-[#b5b5b5] flex-col gap-[20px] p-[20px] md:w-[80%] rounded-lg' >
    <h1 className='text-[18px]'>{star} Out of 5 Rating</h1>
<Star onRate={handelStar}/>
<button className='px-[30px] py-[10px] bg-[red] text-white text-[18px] md:px-[100px] rounded-lg
text-nowrap' onClick={()=> handelRatings(cardDetails._id) }>Submit</button>
</div>

<button className='px-[30px] py-[10px] bg-[red] text-white text-[18px] md:px-[100px] rounded-lg
text-nowrap absolute top-[10px] right-[20px]' onClick={()=> 
